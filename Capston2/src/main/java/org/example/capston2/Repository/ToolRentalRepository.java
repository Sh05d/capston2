...
public interface ToolRentalRepository extends JpaRepository<ToolRental,Integer> {
    ...
    @Query("select r from ToolRental r where r.toolKitId=?1 and r.startDate <= ?3 and r.endDate >= ?2 and r.status!='CANCEL'and r.status!='RETURNED'")
    List<ToolRental> toolRentsAtDate(Integer toolKitId, LocalDate startDate, LocalDate endDate);
