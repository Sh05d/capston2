...
public interface WorkshopRepository extends JpaRepository<Workshop,Integer> {
    ...
    @Query("select w from Workshop w where w.category=?1 and w.startDate between ?2 and ?3 and w.status='SCHEDULE'")
    List<Workshop> availableWorkshopByCategoryAndDate(String category, LocalDateTime firstDate, LocalDateTime secondDate);
    ...
    List<Workshop> findWorkshopByStatus(String status);
}
